...
class WpUser extends Model
{
    ...
    public function scopeSiteRole($query, ?array $filters, ?array $sort)
    {
        if(is_array($filters)){
            foreach ($filters as $filter => $value) {
                switch ($filter) {
                    case 'role':
                        $query->with(['sites', "sites.roles"=> function ($query) use ($value) {
                            $query->where(DB::raw('lower(name)'), 'like', '%'.strtolower($value).'%');
                        }]);                     
                        break;
                    case 'site':
                        $query->with(['sites'=> function ($query) use ($value) {
                            $query->where(DB::raw('lower(name)'), 'like', '%'.strtolower($value).'%');
                        }, "sites.roles"]);
                        break;
                    default:
                        $query->with("sites.roles");
                        break;
                }
            }
        }
        ...
        if(is_array($sort)) {
            ...
            $query->when(
                ...
                static function ($query, $field) use ($direction): void {
                   match($field) {
                       'role' => $query->with(['sites', "sites.roles"=> function ($query) use ($direction){
                                     $query->orderBy('id', !empty($direction) ? $direction : 'ASC');
                                  }]), 
                       'site' =>  $query->with(['sites'=> function ($query) use ($direction){
                                        $query->orderBy('id', !empty($direction) ? $direction : 'ASC');
                                    }, "sites.roles"]),
                       default => $query->with("sites.roles")
                   };
               }
           );
        }
